...
// This function will take search params as we use them in the integration-manager
// and output them into a format usable by the specific connector (ocapi).
/* * *  *  * *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  * */
/* Copyright (c) 2017 Mobify Research & Development Inc. All rights reserved. */
/* * *  *  * *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  * */

var encodeSearchParams = function encodeSearchParams(_ref) {
    ...
    // TODO: sort should become sorts/sorters because we need to handle the case
    // where there are multiple sorting methods
    var params = [];
    ...
    if (filters) {
        var parts = (0, _keys2.default)(filters).map(function (key) {
            return key + '=' + filters[key];
        });
        // Ignore empty filter objects
        parts.length && params.push('refine=' + encodeURIComponent(parts.join(',')));
    }
...
var searchProducts = exports.searchProducts = function searchProducts(searchParams) {
    return function (dispatch) {
        ...
        var options = encodeSearchParams(searchParams).join('&');

        return (0, _utils.makeApiRequest)('/product_search?expand=images,prices,availability' + (options ? '&' + options : ''), { method: 'GET' }).then(function (res) {
            return res.json();
