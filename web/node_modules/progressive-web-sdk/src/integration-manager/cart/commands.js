...
export const removeFromCart = (itemId) => (dispatch, getState) => {
    ...
    return dispatch(connector.removeFromCart(itemId)).then((cart) => {
        dispatchCartAnalytics(EVENT_ACTION.removeFromCart, dispatch, getState, productId, currentQuantity)
        return cart
    })
}
...
export const updateCartItem = (itemId, quantity, productId, variant) => (dispatch, getState) => {
    ...
    return dispatch(connector.updateCartItem(itemId, quantity, productId, variant)).then((cart) => {
        if (deltaQuantity > 0) {
            dispatchCartAnalytics(EVENT_ACTION.addToCart, dispatch, getState, productId, deltaQuantity)
        } else if (deltaQuantity < 0) {
            dispatchCartAnalytics(EVENT_ACTION.removeFromCart, dispatch, getState, productId, -deltaQuantity)
        }
        return cart
    })
}
