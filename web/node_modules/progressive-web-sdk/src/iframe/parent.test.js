...
let frame = new FrameBridge({
    origin: 'about:blank' // default is window.location.origin, which is null
})

describe('with the start method mocked', () => {
    let _startMock
    const originalStart = frame._start
    ...
    test('is created only once with its constructor, even if called multiple times', () => {
        ...
        expect(Object.keys(frame.listeners)).toContain(DEFAULT_EVENTS.CHILD_READY, DEFAULT_EVENTS.CHILD_NAVIGATING)
        expect(frame.eventQueue instanceof Queue).toBe(true)

        frame = new FrameBridge()

        // _start method should only be called once, in the constructor at top of file
        expect(_startMock).not.toBeCalled()
    })
})
...
describe('with the _sendMessage method mocked', () => {
    ...
    const oldSendMessage = frame._sendMessage
    ...
    afterEach(() => {
        frame._sendMessage = oldSendMessage
    })
...
test('trigger calls the Frame prototype trigger method if the FrameBridge is ready', () => {
    // We mocked out sendMessage in the test above, so we need to reset it
    // back to it's original value here
    frame._childReadyHandler()

    const foo = 'foo'
    const mockMessage = jest.fn()
    const _postMessage = frame.childFrame.contentWindow.postMessage
    frame.childFrame.contentWindow.postMessage = mockMessage

    frame.trigger(foo)

    expect(mockMessage).toBeCalledWith(createMockEvent(foo), 'about:blank')
    frame.childFrame.contentWindow.postMessage = _postMessage
})

test('_navigateHandler sets FrameBridge to not ready', () => {
...
describe('with the trigger and on methods mocked', () => {
    const oldTrigger = frame.trigger
    let mockTrigger

    const oldOn = frame.on
    let mockOn
    ...
    test('the callMethod triggers a postMessage to the child, and listens for postMessages from child', () => {
        return frame
            ...
            .then((event) => {
                ...
                expect(mockOn.mock.calls[0]).toContain(`${DEFAULT_EVENTS.RPC_CALL}:10`)
                ...
                expect(event.data).toBe(`${DEFAULT_EVENTS.RPC_CALL}:10`)
            })
    })
})
