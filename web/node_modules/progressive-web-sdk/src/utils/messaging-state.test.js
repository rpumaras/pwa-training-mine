...
describe('MessagingState', () => {
    ...
    const assertCountdowns = (countdowns) => {
        expect(global.localStorage.getItem(`pw-${Const.VISIT_COUNTDOWNS}`)).toEqual(JSON.stringify(countdowns))
    }
    ...
    test('getVisitCountdowns gets countdowns from storage', () => {
        ...
        global.localStorage.setItem(`pw-${Const.VISIT_COUNTDOWNS}`, JSON.stringify(countdowns))
    ...
    test('decreaseVisitCountdowns does not decrease counts past 0', () => {
        // Populate the store with an existing countdowns object
        global.localStorage.setItem(`pw-${Const.VISIT_COUNTDOWNS}`, JSON.stringify({foo: 0}))
    ...
    test('getPageCount returns the page count from storage', () => {
        ...
        global.localStorage.setItem(`pw-${Const.PAGE_COUNT}`, JSON.stringify(1))
    ...
    test('setPageCount sets given page count in storage', () => {
        ...
        expect(global.localStorage.getItem(`pw-${Const.PAGE_COUNT}`)).toBe(JSON.stringify(pageCount))
    })

    test('updateState gets the page count and countdowns, and increases page count by 1', () => {
        // Set existing state in storage
        global.localStorage.setItem(`pw-${Const.VISIT_COUNTDOWNS}`, JSON.stringify({foo: 3}))
        global.localStorage.setItem(`pw-${Const.PAGE_COUNT}`, JSON.stringify(5))
