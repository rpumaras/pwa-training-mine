...
import {doOneOrMany, extractRoute, findRegexpMatches, patternToRegex, extractRouteRegexes, recordRoutes} from './record-routes'
...
test('extractRoute extracts from nested Routes', () => {
    const routes = extractRoute(EXAMPLE_ROUTE)
...
test('extractRouteRegexes creates an array of regexes', () => {
    const arrayToString = (arr) => arr.slice().sort().toString() // eslint-disable-line newline-per-chained-call
    const extractedRegexArray = extractRouteRegexes(EXAMPLE_ROUTE)
    const extractedRegexAsString = arrayToString(extractedRegexArray)
    const expectedRegexAsString = arrayToString(EXAMPLE_ROUTE_REGEX)
...
test('recordRoutes calls its function', () => {
    ...
    recordRoutes(func, EXAMPLE_ROUTE)
