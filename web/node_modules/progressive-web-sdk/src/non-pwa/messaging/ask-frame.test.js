...
const defaultConfiguration = {
    html: `<html><head></head><body><div class="${Const.BUTTON_CONTAINER_CLASS}">` +
...
describe('create', () => {
    ...
    test('constructs an iframe with the right properties', () => {
        ...
        for (const style in expectedStyles) { // eslint-disable-line guard-for-in
...
describe('_clickHandler', () => {
    ...
    describe(`handles click event from ${Const.BUTTON_ALLOW_ID} when`, () => {
        ...
        test('state.subscribed && state.canSubscribe', () => {
            ...
            window.Mobify.WebPush.PWAClient.subscribe.mockReturnValue(Promise.resolve({
                subscribed: true,
                canSubscribe: true
            }))
            ...
            return window.Mobify.WebPush.PWAClient.subscribe().then(() => {
                ...
                expect(askFrame.logger.log.mock.calls).toEqual([['[Messaging UI]', 'Subscribed to channel foo-bar']])
            })
        })
        ...
        test('!state.subscribed && !state.canSubscribe', () => {
            ...
            return window.Mobify.WebPush.PWAClient.subscribe().then(() => {
                expect(askFrame.logger.log.mock.calls).toEqual([['[Messaging UI]', 'Permissions blocked by user; can no longer subscribe.']])
            })
        })
        ...
        test('!state.subscribed && state.canSubscribe', () => {
            ...
            return window.Mobify.WebPush.PWAClient.subscribe().then(() => {
                expect(askFrame.logger.log.mock.calls).toEqual([['[Messaging UI]', 'System ask was dismissed']])
            })
        })
    })
})
