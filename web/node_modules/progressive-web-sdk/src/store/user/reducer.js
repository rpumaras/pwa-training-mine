...
const userReducer = handleActions({
    ...
    [removeWishlistItem]: (state, {payload: {removeItemId}}) => {
        ...
        return state.setIn(
            ...
            state.getIn(wishlistProductsPath).filter(
                (item) => item.get('itemId') !== removeItemId)
            )
    },
    ...
    [receiveUpdatedWishlistItem]: (state, {payload: {itemId, quantity}}) => {
        ...
        const productIndex = wishlistProducts.findIndex((product) => product.get('itemId') === itemId)

        const updatedWishlistProducts = wishlistProducts.setIn([productIndex, 'quantity'], quantity)
