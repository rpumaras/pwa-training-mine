...
export default handleActions({
    ...
    [clearOfflinePages]: (state) => state.set('offlinePageViews', initialState.get('offlinePageViews')),
    [trackOfflinePage]: (state, {payload: {url, routeName, title}}) => {
        const inCache = state.get('fetchedPages').filter((fetchedURL) => fetchedURL === urlToPathKey(url)).size > 0
        return state.update('offlinePageViews', (offlinePageViews) => offlinePageViews.push({url, routeName, title, inCache}))
    }
}, initialState)
