...
test('setPageFetchError sets the error', () => {
    ...
    const finalState = reducer(
        initialState,
        actions.setPageFetchError({error: true})
    )

    expect(selectors
           .getPageFetchError({offline: finalState})
           .equals(Immutable.Map({error: true}))
          ).toBe(true)
})
...
test('getFetchedPages adds to the set of fetched pages', () => {
    ...
    const stateWithOnePage = reducer(
        initialState,
        actions.setFetchedPage('/')
    )

    expect(selectors.getFetchedPages({offline: stateWithOnePage}).has('/')).toBe(true)

    const stateWithTwoPages = reducer(
        stateWithOnePage,
        actions.setFetchedPage('/test')
    )
    ...
    const stateWithDuplicatePages = reducer(
        stateWithTwoPages,
        actions.setFetchedPage('/')
    )
...
test('clearOfflineModeStartTime removes the offline mode start time', () => {
    ...
    const finalState = reducer(
        stateWithStartTime,
        actions.clearOfflineModeStartTime()
    )
...
test('setOfflineModeStartTime sets the offline mode start time', () => {
    ...
    const finalState = reducer(
        initialState,
        actions.setOfflineModeStartTime(startTime)
    )
...
test('trackOfflinePage sets inCache to true if page has been fetched already', () => {
    ...
    const stateWithFetchedPages = reducer(
        initialState,
        actions.setFetchedPage(pageData.url)
    )

    const finalState = reducer(
        stateWithFetchedPages,
        actions.trackOfflinePage(pageData)
    )
