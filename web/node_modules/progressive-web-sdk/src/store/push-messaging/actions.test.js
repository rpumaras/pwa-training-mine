...
// Need to define this here, rather than `beforeAll` as that happens _after_ any
// import statements
global.localStorage = {
    _storedKeys: {},
    _removeAll() {
    ...
    getItem(key) {
        return this._storedKeys[key]
    },
    setItem(key, value) {
        this._storedKeys[key] = value
    },
    removeItem(key) {
        delete this._storedKeys[key]
    }
}
...
afterEach(() => {
    global.localStorage._removeAll()
...
test('setVisitCountdownsInStorage updates local storage with provided visitCountdowns from store', () => {
    ...
    expect(global.localStorage.getItem('pw-visitCountdowns')).toBe(JSON.stringify(countdowns))
})
...
test('channelOfferShown calls MessagingClient channelOfferShown with the shown channel name', () => {
    ...
    return store.dispatch(actions.channelOfferShown(channelName)).then(() => {
        expect(window.Progressive.MessagingClient.channelOfferShown.mock.calls[0][0]).toEqual(channelName)
    })
})
...
test('channelOfferShown sends an undefined channel value if none provided', () => {
    ...
    return store.dispatch(actions.channelOfferShown()).then(() => {
        expect(window.Progressive.MessagingClient.channelOfferShown.mock.calls[0][0]).toBeUndefined()
    })
})
...
test('incrementPageCount dispatches onPageCountIncrement and stores page count in local storage', () => {
    ...
    expect(store.getActions()).toEqual(expectedActions)
    expect(global.localStorage.getItem('pw-pageCount')).toBeDefined()
})
...
test('rehydratePageCount dispatches onRehydratedPageCount with value of pageCount from local storage', () => {
    global.localStorage.setItem('pw-pageCount', 3)
    const store = mockStore({})
...
test('rehydrateVisitCountdowns dispatches onRehydratedVisitCountdowns with value of visitCountdowns from local storage', () => {
    global.localStorage.setItem('pw-visitCountdowns', JSON.stringify({
        default: 3,
        'new-deals': 4
    }))
    const store = mockStore({
        pushMessaging: Immutable.fromJS({
            [constants.VISIT_COUNTDOWNSS]: {}
        })
    })

    const expectedActions = [actions.onRehydratedVisitCountdowns({
        default: 3,
        'new-deals': 4
    })]
...
describe('notificationClick', () => {
    ...
    test('includes query string and hash, if present', () => {
        ...
        expect(browserHistory.push.mock.calls[0][0]).toEqual('/potions.html?hello=world&foo=bar#anchor-element')
        expect(window.location.href).toEqual('http://www.bar.com')
    })
...
test('setLocale does not accept non- or empty strings, and logs as such', () => {
    ...
    return store.dispatch(actions.setLocale()).then(() => {
        expect(console.log.mock.calls[0]).toEqual(['[Messaging UI]', 'Locale must be specified as a string.'])
...
test('setVisitEndTimestamp updates the value of visitEndTimestamp to 6 hours in the future', () => {
    ...
    expect(actions.getVisitEndTimestamp()).toEqual(date.getTime() + (6 * 60 * 60 * 1000))
})
