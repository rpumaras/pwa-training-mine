...
import {PAGE_COUNT, VISIT_COUNTDOWNS, VISITS_TO_WAIT, CHANNEL_NAME, DEFAULT_CHANNEL, ACTIVE_VISIT_DURATION, PERMA_DURATION} from './constants'

export const onRehydratedPageCount = createAction('[Push Messaging] Page Count Rehydrated')
export const onRehydratedVisitCountdowns = createAction('[Push Messaging] Visit Countdowns Rehydrated', [VISIT_COUNTDOWNS])
...
export const onVisitCountdownStarted = createAction('[Push Messaging] Visit Countdown Set', [VISITS_TO_WAIT, CHANNEL_NAME])
export const messagingSystemAskShown = createAction('[Push Messaging] System Ask Shown/Not Shown', ['systemAskShown'])
export const setVisitsToWaitIfDismissed = createAction('[Push Messaging] Set Visits to Wait when Dismissed', ['visitsToWaitIfDismissed'])
...
export const setVisitEndTimestamp = (durationOverride) => () => {
    const date = new Date()
    const duration = typeof durationOverride !== 'undefined' ? durationOverride : ACTIVE_VISIT_DURATION
...
export const channelOfferShown = (channel) => (dispatch) => {
    ...
    return window.Progressive.MessagingClientInitPromise
        .then(() => {
            logger.log(`Notifying Messaging Client that channel ${channel || DEFAULT_CHANNEL} was displayed.`)
