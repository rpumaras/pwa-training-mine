...
test('NavSlider renders without errors', () => {
    const wrapper = shallow(<NavSlider><h1>content</h1></NavSlider>)
    expect(wrapper.html()).toBe(
        '<div class="pw-nav-slider">' +
            '<h1>content</h1>' +
        '</div>'
    )
})
...
test('renders the contents of the className prop if present', () => {
    const name = 'name'
    const wrapper = shallow(<NavSlider className={name}><h1>content</h1></NavSlider>)
    expect(wrapper.html()).toBe(
        '<div class="pw-nav-slider name">' +
            '<h1>content</h1>' +
        '</div>'
    )
})
...
test('animates on enter', () => {
    return Promise.all(['descending', 'ascending'].map((action) => {
        ...
        return new Promise((resolve) => {
            instance.componentWillEnter(resolve)
        }).then(() => {
            const expected = {
                ...
                transition: '-ms-transform 0.5s ease-out,-webkit-transform 0.5s ease-out,transform 0.5s ease-out'
            }
