...
const renderFullTree = (root = {}, expanded = {}, getRealPath, alreadyRenderedExpandedPanel) => {
    ...
    if (root.path === expanded.path) {
        ...
        result = immediateChildren.reduce((acc, {children}) => {
            ...
            return acc.concat(children.map((child) => renderFullTree(child, expanded, getRealPath, true)))
        }, [])
    } else {
        let childNodes = []
        if (root.children) {
            childNodes = root.children.map((child) => renderFullTree(child, expanded, getRealPath, alreadyRenderedExpandedPanel))
        }

        result = [
            (<a href={getRealPath(root.path)} key={root.path}>{root.title}</a>),
            ...childNodes
        ]
    }
