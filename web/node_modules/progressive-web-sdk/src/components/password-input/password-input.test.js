...
test('toggle button has `pw--is-text` class when is `isText` true ', () => {
    const wrapper = shallow(<PasswordInput isText />)
...
test('tapping the button toggles the text for toggle button', () => {
    ...
    const wrapper = mount(<PasswordInput isText />)

    // Get the inner `<button>`, but not the outter `<Button>`
