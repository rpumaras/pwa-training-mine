...
describe('PushMessagingController', () => {
    ...
    test('renders a Sheet component correctly; closed by default', () => {
        const wrapper = shallow(<PushMessagingController {...mocks} />, {disableLifecycleMethods: true})
    ...
    test('renders an open Sheet if conditions are met', () => {
        const wrapper = shallow(<PushMessagingController dimScreenOnSystemAsk={true} {...mocks} isSystemAskShown={true} />, {disableLifecycleMethods: true})
        expect(wrapper.props().open).toBeTruthy()
    })

    test('does not open Sheet if isSubscribed is true, despite other matching conditions', () => {
        const wrapper = shallow(<PushMessagingController dimScreenOnSystemAsk={true} {...mocks} isSystemAskShown={true} isSubscribed={true} />, {disableLifecycleMethods: true})
        expect(wrapper.props().open).toBe(false)
    })

    test('shouldComponentUpdate returns false if `dimScreenOnSystemAsk` is false', () => {
        const wrapper = shallow(<PushMessagingController dimScreenOnSystemAsk={false} {...mocks} />, {disableLifecycleMethods: true})
    ...
    test('shouldComponentUpdate returns true only when `isSystemAskShown` changes if `dimScreenOnSystemAsk` is true', () => {
        const wrapper = shallow(<PushMessagingController dimScreenOnSystemAsk={true} {...mocks} />, {disableLifecycleMethods: true})
    ...
    test('shouldComponentUpdate returns false if `canSubscribe` is false', () => {
        const wrapper = shallow(<PushMessagingController {...mocks} canSubscribe={false} />, {disableLifecycleMethods: true})
    ...
    test('componentWillMount calls setup methods', () => {
        shallow(<PushMessagingController {...mocks} canSubscribe={false} />, {disableLifecycleMethods: true})
    ...
    describe('methods which use the default PushMessagingController component', () => {
        ...
        beforeEach(() => {
            wrapper = shallow(<PushMessagingController {...mocks} />, {disableLifecycleMethods: true})
        ...
        describe('componentDidMount', () => {
            test('logs error message and updates status in push messaging store if Promise is undefined', () => {
                ...
                return instance.componentDidMount().then((returnValue) => {
                    expect(console.error.mock.calls[0][0]).toEqual('[Messaging UI] Could not initialize the Messaging Client.')
            ...
            test('logs error message and updates status to FAILED if failed to initialize', () => {
                window.Progressive = {}
                window.Progressive.MessagingClientInitPromise = jest.fn().mockReturnValue(Promise.reject())()
                ...
                return instance.componentDidMount().then((returnValue) => {
                    ...
                    expect(mocks.stateUpdate.mock.calls[0][0]).toEqual({status: MESSAGING_STATUS.FAILED})
                })
            })

            test('calls MessagingClient.register with handleStateUpdate and handleNotificationClick, then calls stateUpdate', () => {
                window.Progressive = {}
                window.Progressive.MessagingClientInitPromise = jest.fn().mockReturnValue(Promise.resolve())()
                window.Progressive.MessagingClient = {
                ...
                return instance.componentDidMount().then(() => {
                    ...
                    expect(window.Progressive.MessagingClient.register.mock.calls[0][0]).toBe(instance.handleNotificationClick)
                    expect(window.Progressive.MessagingClient.register.mock.calls[0][1]).toEqual(window.Progressive.MessagingClient.Events.notificationClick)
                    expect(window.Progressive.MessagingClient.register.mock.calls[1][0]).toBe(instance.handleStateUpdate)
                    expect(window.Progressive.MessagingClient.register.mock.calls[1][1]).toBe(window.Progressive.MessagingClient.Events.messagingStateChange)
                    expect(mocks.stateUpdate.mock.calls.length).toBe(1)
                    expect(mocks.stateUpdate.mock.calls[0][0]).toEqual({foo: 'bar', isReady: true, status: MESSAGING_STATUS.READY})
                })
            })
        })
