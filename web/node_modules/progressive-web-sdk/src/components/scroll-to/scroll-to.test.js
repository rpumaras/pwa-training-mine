...
test('renders the contents of the className prop if present', () => {
    [
        ...
        const wrapper = shallow(<ScrollTo className={name} target="/">Test</ScrollTo>)
...
test('will scroll to the specified numeric position immediately with zero duration', () => {
    const wrapper = shallow(<ScrollTo target={50} duration={0}>Test</ScrollTo>)
...
test('will not scroll if the selector matches nothing', () => {
    const wrapper = mount(<ScrollTo target=".nonexistent-class" duration={0}>Test</ScrollTo>)
...
test('will scroll to an element if the selector matches one', () => {
    ...
    const wrapper = mount(<ScrollTo target=".target" duration={0}>Test</ScrollTo>)
    wrapper.simulate('click')
...
test('will animate if the duration is provided', () => {
    const wrapper = mount(<ScrollTo target={1000} duration={250}>Test</ScrollTo>)
