...
describe('withPushMessaging higher order component', () => {
    ...
    describe('componentWillReceiveProps', () => {
        ...
        test('calls channelOfferShown if `shouldAsk` returned true during a change in state', () => {
            wrapper = mount(<WrapperComponent store={store} {...noops} channelOfferShown={channelOfferShown} channelName="bar" showOnPageCount={false} />)
        ...
        test('does not call setState if state change would be identical', () => {
            wrapper = mount(<WrapperComponent store={store} {...noops} channelOfferShown={channelOfferShown} />)
        ...
        test('does not call channelOfferShown during a state change, if shouldAskResult is false', () => {
            wrapper = mount(<WrapperComponent store={store} {...noops} channelOfferShown={channelOfferShown} showOnPageCount={false} />)
    ...
    describe('class method', () => {
        ...
        const getSubscribeMock = (didSubscribe, canSubscribe) => jest.fn(() => new Promise((resolve) => {
            resolve({
        ...
        test('`startVisitCountdown` only calls startVisitCountdown if deferOnDismissal is > 0', () => {
            wrapper = mount(<WrapperComponent store={store} {...noops} channelName="foo" startVisitCountdown={startVisitCountdown} />)
        ...
        test('`dismissed` calls the startVisitCountdown method', () => {
            ...
            return wrappedComponent.props().dismissed().then(() => {
        ...
        test('`accepted` calls subscribe with a provided channelName prop', () => {
            wrapper = mount(<WrapperComponent store={store} {...noops} subscribe={subscribe} channelName="bar" />)
            wrappedComponent = wrapper.find(mockComponent.displayName)

            return wrappedComponent.props().accepted().then(() => {
        ...
        describe('`accepted`', () => {
            ...
            test('Logs the channel name properly if user accepts system-ask', () => {
                subscribe = getSubscribeMock(true)
                wrapper = mount(<WrapperComponent store={store} {...noops} subscribe={subscribe} channelName="foo" />)
                wrappedComponent = wrapper.find(mockComponent.displayName)
            ...
            test('logs appropriately when user is unable to be subscribe', () => {
                ...
                return wrappedComponent.props().accepted().then(() => {
                    expect(console.log.mock.calls[0][1]).toEqual('Permissions blocked by user; can no longer subscribe.')
                })
            })
        })
    })
