...
test('renders a visible title if the icon and title props are passed with showIconText set to true', () => {
    ...
    expect(wrapper.find('span').first().hasClass('pw-button__text')).toBe(true)
    expect(wrapper.find('span').first().text()).toBe(title)
})
...
test('HandlerClick sends analytics data', () => {
    const mockOnClick = jest.fn()
    const mockButtonName = 'mockName'
    const mockButtonContent = 'mockContent'
    const wrapper = shallow(<Button
        onClick={mockOnClick}
        data-analytics-name={mockButtonName}
        data-analytics-content={mockButtonContent}
    />)
    ...
    expect(window.Progressive.analytics.send).toBeCalledWith({
        [UIInteraction.NAME]: mockButtonName,
        [UIInteraction.CONTENT]: mockButtonContent,
