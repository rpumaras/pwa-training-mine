...
// `pw-grid__span.pw--span-1@mobile`, and so on. See the below "Grid Modifier"
// section for more detail.

@mixin grid__generate-modifiers($columnCount, $breakpoint) {
    @for $i from 1 through $columnCount {
        .pw-grid__span.pw--span-#{$i}\@#{$breakpoint} {
            @include span($i);
        }
    }

    @for $i from 1 through ($columnCount - 1) {
        .pw-grid__span.pw--pre-#{$i}\@#{$breakpoint} {
            @include pre($i);
        }

        .pw-grid__span.pw--post-#{$i}\@#{$breakpoint} {
            @include post($i);
        }
    }

    .pw-grid__span.pw--full-width\@#{$breakpoint} {
        @include full;
    }
}
...
//
// 1. Overflow the `.pw-grid__inner` container to offset the first and last
...
// ### Spans
//
// For example `pw-grid__span.pw--span-4@mobile` is a container that spans four
// columns of the grid. It applies this style on only the "mobile" breakpoint.
//
// ### Pre
//
// For example `pw-grid__span.pw--pre-4@tablet` adds an amount of `margin` equal
...
// ### Post
//
// For example `pw-grid__span.pw--pre-4@desktop` adds an amount of `margin` equal
...
// ### Full-Width
//
// For example `pw-grid__span.pw--full-width@mobile` sets the container to span
