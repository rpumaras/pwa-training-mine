...
export const sendOfflinePageview = createActionWithAnalytics(
    ...
    [],
    EVENT_ACTION.pageview,
    (url, routeName, title, offlineSuccess) => (new Page({
        [Page.LOCATION]: url,
...
const sendAddToCartAnalytics = createActionWithAnalytics('Send cart analytics', [], EVENT_ACTION.addToCart, createCartAnalyticsMeta)
const sendRemoveFromCartAnalytics = createActionWithAnalytics('Send cart analytics', [], EVENT_ACTION.removeFromCart, createCartAnalyticsMeta)
...
export const dispatchCartAnalytics = (action, dispatch, getState, productId, quantity) => {
    ...
    const actionCreator = action === EVENT_ACTION.addToCart ? sendAddToCartAnalytics : sendRemoveFromCartAnalytics
...
const sendAddToWishlistAnalytics = createActionWithAnalytics('Send wishlist analytics', [], EVENT_ACTION.addToWishlist, createWishlistAnalyticsMeta)
const sendRemoveFromWishlistAnalytics = createActionWithAnalytics('Send wishlist analytics', [], EVENT_ACTION.removeFromWishlist, createWishlistAnalyticsMeta)
    ...
    if (fields) {
        Object.keys(fields).forEach((fieldName) => {
            if (fieldName === '_error') {
                ...
                if (element && hasAnalyticsNameAttribute(element, 'Element', analyticsManager.options.debug)) {
...
export const ReduxFormPluginOption = {
    all: (state, action) => {
        ...
        try {
            if (action.type === actionTypes.STOP_SUBMIT) {
            ...
            } else if (lastUpdateSyncError &&
                action.meta && action.meta.field &&
                ...
                sendFormFieldValidationErrorsAnalytics({
                    ...
                    fields: {
                        [action.meta.field]: lastUpdateSyncError.payload.syncErrors[action.meta.field]
                    }
                })
...
export const sendProductImpressionAnalytics = createActionWithAnalytics(
    'Send product impression analytics', [], EVENT_ACTION.productImpression,
    (productId) => ({productId})
)
...
export const sendProductDetailAnalytics = createActionWithAnalytics(
    'Send product detatil analytics', [], EVENT_ACTION.productDetail
)
...
export const sendApplePayOptionDisplayedAnalytics = createActionWithAnalytics(
    'Send Apple Pay option displayed analytics', [], EVENT_ACTION.applePayOptionDisplayed
)

export const sendApplePayButtonDisplayedAnalytics = createActionWithAnalytics(
    'Send Apple Pay button displayed analytics', [], EVENT_ACTION.applePayButtonDisplayed
)

export const sendApplePayButtonClickedAnalytics = createActionWithAnalytics(
    'Send Apple Pay button clicked analytics', [], EVENT_ACTION.applePayButtonClicked
)
