...
const assertValidConnector = (connector) => {
    if (!(Object.getPrototypeOf(connector) instanceof Connector)) {
        throw new Error(
            `Analytics connector must be an instance of Connector`
        )
    }
}
...
const captureEventsFromDOM = (analyticsManager) => {
    ...
    rootEl.addEventListener('focus', (event) => {
        ...
        /* istanbul ignore else */
        if (!element.hasAttribute(DATA_TOUCHED) &&
            (NODE_NAMES.hasOwnProperty(nodeName.toLowerCase()) || element.hasAttribute(DATA_NAME))
        ) {
            if (nodeName === NODE_NAMES.input && !(element.type === 'radio' || element.type === 'checkbox') || nodeName === NODE_NAMES.textarea) {
                ...
                element.addEventListener(nodeName === NODE_NAMES.select ? 'change' : 'click', () => {
                    captureUIInteractions(analyticsManager, element)
                }, EVENT_OPTIONS)
            }
        }
    ...
    if (window.Progressive) {
        ...
        window.Progressive.analytics.send = (analyticsData) => {
            analyticsManager.distribute(EVENT_ACTION.uiInteraction, new UIInteraction(analyticsData, [], {
...
class AnalyticsManager {
    ...
    collectPerformance(type, value) {
        ...
        if (type !== PERFORMANCE_METRICS.templateWillMount && !performanceTiming.templateWillMount) {
            performanceTiming = {}
            assetCount = 0
            return
        }
        ...
        if (performanceTiming.templateWillMount &&
    ...
    distribute(event, metaPayload, state) {
        ...
        this.connectors.forEach((connector) => {
            try {
                ...
                this.engagementEngine.receive(EVENT_ACTION.uiInteraction, new UIInteraction({
                    ...
                    [UIInteraction.CONTENT]: err.stack ? err.stack.substr(0, MAX_CONTENT_FIELD_LENGTH) : err.toString().substr(0, MAX_CONTENT_FIELD_LENGTH)
                }))
            }
        })
    }
