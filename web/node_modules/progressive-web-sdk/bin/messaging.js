...
const getPackageOptions = (argv) => {
    ...
    const options = Object.assign(
        ...
        packageOptions,
        {
            ...
            configFilePath:
                `${messagingPath}/${packageOptions.messagingSiteId}.yaml`,
            argv
        }
    )
    ...
    if (!siteId) {
        Utils.fail(
            `[Error: The messagingSiteId value must be defined in ${options.packagePath}]`
        )
    }

    if (!fs.existsSync(options.configFilePath)) {
        Utils.fail(
            `[Error: ${options.configFilePath} not found]`
        )
    }
...
const argv = yargs // eslint-disable-line no-unused-vars
    ...
    .command(
        'upload',
        'upload the Messaging configuration files',
        {},
        (args) => {
            MessagingUtils.uploadConfig(getPackageOptions(args))
            .catch(errorCatcher)
        }
    )
    .command(
        ...
        {},
        (args) => {
            MessagingUtils.generateCSR(getPackageOptions(args))
                .catch(errorCatcher)
        }
    )
    .command(
        ...
        (args) => {
            MessagingUtils.sendTestMessage(getPackageOptions(args))
                .catch(errorCatcher)
        }
    )
    .option(POTENTIAL_OPTIONS.projectDirectory, {
        alias: 'projectDirectory',
        describe: 'the directory where your project is located. The default is the current directory'
    })
    // hidden option
