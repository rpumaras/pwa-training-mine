        ...
        (resolve, reject) => {
            ...
            archiveStream.on(
                ...
                () => {
                    ...
                    const optionsWithData = Object.assign(
                        {},
                        options,
                        {
...
const getCredentials = (options) => {
    return Utils.readCredentials(Utils.getSettingsPath())
        .then(
            (credentials) => {
        ...
        .then(
            ...
            (data) => {
                ...
                const output = [
                    ...
                    `${fileName.replace('.csr', '.cer')} and move it to the ${options.messagingPath} folder.`,
...
const waitUntilDelivered = (options) => {
    ...
    const checker = (resolve, reject) => makeRequest(requestOptions)
        .then(JSON.parse)
        .then(handleV2APIResponse)
        .then((deliveryResponse) => {
            if (deliveryResponse.state === DELIVERED) {
                console.log(
                `Delivery ${options.deliveryId} now marked as DELIVERED`
                )
...
const deliverTestMessage = (options) => {
    ...
    return makeV2APIRequest(requestOptions)
        .then((deliveryResponse) => {
            console.log(
                `Message sent as delivery ${deliveryResponse.delivery_id}`
            )
