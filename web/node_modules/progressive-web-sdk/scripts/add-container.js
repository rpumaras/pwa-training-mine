...
const USER_INPUT_SCHEMA = [
    {
        ...
        message: 'The name must begin with a upper-case letter, and only contain letters and numbers.' // eslint-disable-line max-len
    }
]
...
const makeContainerDir = (container) => {
    return fs.mkdirAsync(container.path)
...
const addContainer = () => {
    common.getUserInput(USER_INPUT_SCHEMA)
        ...
        .tap((container) => {
            return common.step(
                'Updating templates.js',
                updateTemplates
            )(container)
        })
        .then(common.step(
            'Finding container template',
            common.findTemplateFilenames(skeletonDir)
        ))
        .tap(() => console.log('Generating container:'))
        .spread((container, filenames) => {
            return Promise.map(filenames, common.fileTransformer(
                container,
                skeletonDir
            ))
        })
        ...
        .then(() => {
            ...
            console.log(`- Modify 'app/router.jsx' to import the newly generated page from './containers/templates' and use it in a <Route>.`)
            console.log(`- Import your container's .scss file in 'web/app/styles/_templates.scss' and start adding some styles.`)
        })
}
