...
const findTemplateFilenames = (templateDir) => (context) =>
      [context, fileUtils.readdirAsync(asset(templateDir))]
...
const transformFile = (inpath, context, outpath, suppress) => {
    return fileUtils.readFile(asset(inpath))
...
const outputName = (filename, context) => path.join(
    context.path,
    filename.replace('example', context.dirname)
)

const fileTransformer = (context, inDir) => (filename) => {
    return transformFile(
        path.join(inDir, filename),
        context,
        outputName(filename, context)
    )
}
