...
const filterOnStat = (pathBuilder, statCondition) => (items) => {
    return Promise.map(items, (item) => {
        return statAsync(pathBuilder(item))
            .then((stats) => { return statCondition(stats) ? item : null })
            .catchReturn(null)
    })
        .then(clearNulls)
}

const filterDirectories = (pathBuilder) =>
      filterOnStat(pathBuilder, (stats) => stats.isDirectory())

const filterFiles = (pathBuilder) =>
      filterOnStat(pathBuilder, (stats) => stats.isFile())
