...
test('component adds the component path', () => {
    testConverter(common.component, [
        ['this.jsx', `${common.APP_COMPONENT_DIR}/this.jsx`],
        ['my-component/index.jsx', `${common.APP_COMPONENT_DIR}/my-component/index.jsx`]
    ])
})

test('container adds the container path', () => {
    testConverter(common.container, [
        ['reducer.js', `${common.APP_CONTAINER_DIR}/reducer.js`],
        ['my-container/action.test.js', `${common.APP_CONTAINER_DIR}/my-container/action.test.js`]
    ])
})

test('asset adds the asset path', () => {
    testConverter(common.asset, [
        ['reducer.template.js', `${common.ASSET_DIR}/reducer.template.js`],
        ['component-skeleton/example.jsx', `${common.ASSET_DIR}/component-skeleton/example.jsx`]
    ])
})
...
test('getUserInput should get input from prompt passing the schema', () => {
    ...
    return common.getUserInput(schema)
        .then((input) => {
...
test('outputName puts the file in the path', () => {
    expect(common.outputName('test.js', {path: 'path/to/output'})).toBe('path/to/output/test.js')
})

test('outputName replaces \'example\' with the directory name', () => {
    expect(common.outputName('example.js', {path: 'output', dirname: 'test'})).toBe('output/test.js')
})
...
test('fileTransformer creates a transform function with fixed context', () => {
    ...
    const fn = common.fileTransformer({type: 'template', path: 'output', dirname: 'test'}, 'skeleton')
    expect(typeof fn).toBe('function')

    return fn('thing.txt')
        .then(() => {
            expect(fileUtils.readFile).toBeCalled()
            expect(fileUtils.readFile.mock.calls[0][0].endsWith('skeleton/thing.txt')).toBe(true)

            expect(fileUtils.writeToPath).toBeCalled()
            expect(fileUtils.writeToPath).toBeCalledWith('output/thing.txt')
