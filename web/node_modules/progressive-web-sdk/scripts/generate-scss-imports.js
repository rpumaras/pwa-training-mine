...
const pathsFromDirs = (dirs) => dirs.map((dir) => {
    if (fileUtils.existsSync(common.component(path.join(dir, '_theme.scss')))) {
        return {
            basePath: path.join('..', 'src', 'components', dir, '_base.scss'),
            themePath: path.join('..', 'src', 'components', dir, '_theme.scss'),
        }
    } else {
        return {
            basePath: path.join('..', 'src', 'components', dir, '_base.scss'),
...
const getComponents = () => {
    return fs.readdirAsync(common.APP_COMPONENT_DIR)
...
const generateScssImports = () => {
    return Promise.resolve()
        ...
        .then(common.step(
            'Generating component list CSS program text\n',
            (components) => common.transformFile(
...
if (require.main === module) {
    generateScssImports()
        .then(() => consoleOutput.greenWrite('Finished successfully\n'))
}
